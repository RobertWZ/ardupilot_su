...
bool GCS_MAVLINK::init(uint8_t instance)
{
    ...
    if (chan == MAVLINK_COMM_0 || chan == MAVLINK_COMM_1 || chan == MAVLINK_COMM_2) {
        // Always start with MAVLink2 on first port for now, to allow for recovery
        // after experiments with MAVLink1
		status->flags &= ~MAVLINK_STATUS_FLAG_OUT_MAVLINK1;
    }
                                 ...
                                 const mavlink_message_t &msg)
{
    ...
    AP::logger2().Log_Write_MALK((uint8_t)chan, msg.msgid);
    handleMessage(msg);
}
