...
void AC_PosControl::run_z_controller()
{
    ...
    if (_dt > 0.0f) {
        if (!_flags.freeze_ff_z) {
            ...
            _accel_desired.z = _lowpass_filter.apply(_accel_desired.z, _dt);
        } else {
    ...
    if (_flags.reset_rate_to_accel_z) {
    	...
    	float vel_error_z = _vel_target.z - curr_vel.z;
        _vel_error.z = _vel_error_filter.apply(vel_error_z, _dt);
    	AP::logger2().Log_Write_LPVE(vel_error_z, _vel_error.z);
    }
    ...
    AP::logger().Write_PID(LOG_PIDA_MSG, _pid_accel_z.get_pid_info());
    AP::logger2().Log_Write_Alt(_pos_target.z,
    		                    curr_alt,
								_vel_target.z,
								_vel_desired.z,
								curr_vel.z,
